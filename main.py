...
bot = telebot.TeleBot('7788551370:AAHi56ZfhjieWic2OVNSQIcsiALJxTYS4pY')

@bot.message_handler(commands=['start'])
def start_message(message):
    bot.send_message(message.from_user.id, 'Добро пожаловать! Поделитесь, пожалуйста, контактом.',
...
def handle_product_selection(message):
    ...
    if success:
        bot.send_message(user_id, f"Товар '{product_name}' добавлен в вашу корзину.",
                         reply_markup=buttons.buy_product())
    else:
